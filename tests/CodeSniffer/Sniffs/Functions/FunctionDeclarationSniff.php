 ...
 * Based on the PHP_CodeSniffer rule for function declaration 
 */
class CodeSniffer_Sniffs_Functions_FunctionDeclarationSniff implements PHP_CodeSniffer_Sniff
{
     ...
     * @return void
     */
    public function processSingleLineDeclaration(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        //Do not use OpeningFunctionBraceBsdAllmanSniff any more because it counts number of spaces. lol!
	...
	// indent parameters. */
	private function prepareProcessMultiLineDeclaration(&$functionIndent, &$stackPtr, &$tokens)
	{
        $functionIndent = 0;
    ...
    {
        $functionIndent = 0;
		$this->prepareProcessMultiLineDeclaration($functionIndent, $stackPtr, $tokens);
        ...
        for ($i = ($openBracket + 1); $i < $closeBracket; $i++) 
		{
            if ($tokens[$i]['line'] !== $lastLine) 
				...
				{
                    // Closing brace needs to be indented to the same level as the function.
